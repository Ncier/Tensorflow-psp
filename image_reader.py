...
def random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w, ignore_label=255):
    ...
    print("img_crop=","label_crop=",img_crop,label_crop)
...
data_dir = r'D:/绿潮数据集/'
data_list = r'G:/绿潮数据集/绿潮数据集2/80x80/cityscapes_train_list.txt'
...
def create_image_lists(image_dir):
    ...
    for directory in directories:
        ...
        if directory=='images':                                               
            for f in file_list:                
                images.append(f)  
            print ('No. of %s files: %d' % (directory, len(images)))                                         
        else:
                                  
            for f in file_list:                
                masks.append(f)                   
    
            print ('No. of %s files: %d' % (directory, len(masks)))
...
class ImageReader(object):
    ...
    def __init__(self, image_dir,data_dir, data_list, input_size,
        ...
        self.image_list, self.label_list = create_image_lists(image_dir)
